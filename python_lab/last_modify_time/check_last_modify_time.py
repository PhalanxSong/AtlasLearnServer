...
# 检测根目录
check_root_dir = os.getcwd() + '\last_modify_time\ExampleFolder'
# 忽略目录
ignore_dir = check_root_dir + '\IgnoreFolder'
# 记录文件存储目录
save_file_root = os.getcwd() + '\last_modify_time\TempPy'
...
for parent, dirnames, filenames in os.walk(check_root_dir):
    # 跳过 ignore_dir
    if parent.startswith(ignore_dir):
        continue
    # 2.获取 子目录 的 修改时间
    for dirname in dirnames:
        ...
        # 跳过 ignore_dir
        if dir_full_path.startswith(ignore_dir):
            continue
        if modify_time > latest_modify_time:
            latest_modify_time = modify_time
